...
This thesis explores the Learning by Teaching (LbT) paradigm, where children interact with a social robot to foster turn-taking and prosocial behavior. 
...
We conducted three formative, iterative experiments involving HCI experts, high school students, and middle school children, leading to refinements in the robot's behavior and the dashboard interface. 
...
The integration of social robots in educational settings has been explored through various paradigms such as direct instruction, peer-like interaction, and more recently, the \textit{Learning by Teaching (LbT)} approach.
LbT is grounded in the idea that teaching others enhances the learner's own understanding~\cite{biswas2005learning}.
In the context of HRI, this paradigm has shown potential to increase engagement and learning outcomes, particularly with young learners~\cite{10.5898/JHRI.1.1.Tanaka, love2024teachable}.

Several systems have explored how teachable robots can support this dynamic.
For instance, Tanaka and Matsuzoe~\cite{10.5898/JHRI.1.1.Tanaka} demonstrated how Japanese children taught vocabulary to a NAO robot, while Love et al.~\cite{love2023adapting, love2024teachable} explored reinforcement learning to personalize dialog and improve engagement during teaching conversations.

Despite this progress, early-stage experimentation still relies heavily on the \textit{Wizard-of-Oz (WoZ)} method~\cite{weiss2010userWOZ, rietz2021woz4uPepper}, where human operators simulate intelligent behavior to prototype interactions and gather feedback.
Tools like WoZ4U provide open-source dashboards for controlling Pepper robots, yet they are tailored to humanoid platforms and not readily adaptable to other robotics systems like the RoboMaster EP.

Moreover, most existing platforms used in robotics research (e.g., \textit{Foxglove}~\cite{foxglove}, \textit{RViz2}~\cite{rviz2}) are not designed with non-technical users or child-facing applications in mind.
These tools focus on telemetry and debugging rather than real-time social interaction.
Our project addresses this gap by offering an expressive and accessible interface co-designed for child-robot interaction scenarios.

The \textit{TESORO proposal}~\cite{landoni2025tesoro} situates this effort in a broader vision that seeks to empower children, including those with intellectual disabilities, to explore their emotions and self-regulation through collaborative play with robots.
Our work contributes to this vision by prototyping early interaction strategies and a control infrastructure that supports eventual transitions to autonomy and mutual learning.

Finally, recent participatory design studies emphasize the importance of co-designing robot behavior with children~\cite{osti_10386132, rose2019participatory}, reinforcing that accessibility, playfulness, and emotional awareness are key factors in successful child-robot interaction design.
Our iterative evaluation confirms these values and informs both system-level design and future research directions.
